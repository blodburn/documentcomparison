...
7. 내보내기
‘Excel 내보내기’는 비교 결과의 텍스트와 변경표시를 XLSX로 저장합니다. ‘Word 변경추적’은 변경 전 → 변경 후 조합을 선택해 Word 변경추적 문서를 만듭니다. DOCX를 변경 후 문서(B)로 선택하면 B의 표·이미지·스타일·번호·콘텐츠 컨트롤 등 기존 양식을 기준으로 본문 텍스트 변경만 주입합니다. 표 행/셀의 추가·삭제는 Word의 구조 변경추적으로 기록합니다. B에 기존 변경추적이 남아 있거나 A/B의 머리글·바닥글·각주·미주가 서로 달라 현재 안전하게 추적할 수 없는 경우에는 불완전한 파일 생성을 막기 위해 저장을 중단합니다.
...
"""
        : """
Document Compare V5.20.14 · Quick Guide
